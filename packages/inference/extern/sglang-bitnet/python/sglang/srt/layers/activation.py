...
def cpu_has_avx2_or_avx512() -> bool:
    """Check if CPU has AVX2 or AVX512 support."""
    try:
        import cpuinfo
        ...
        try:
            with open('/proc/cpuinfo', 'r') as f:
                cpuinfo_text = f.read()
                return 'avx2' in cpuinfo_text or 'avx512' in cpuinfo_text
        except Exception:
            return False
...
_is_cpu_avx_available = cpu_has_avx2_or_avx512()
_is_cpu = is_cpu()
_is_hip = is_hip()
_is_xpu = is_xpu()
...
if not (
    _is_cuda or _is_npu or (_is_cpu and (_is_cpu_amx_available or _is_cpu_avx_available)) or _is_hip or _is_xpu
):
    logger.info(
        "sgl-kernel is not available on Non-NV, Non-AMD platforms or Non-AMX/AVX CPUs. Fallback to other kernel libraries."
    )
