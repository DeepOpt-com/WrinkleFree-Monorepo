...
Usage:
    ...
    from wrinklefree.teachers import VLLMTeacher
...
from wrinklefree.teachers.local_teacher import LocalTeacher
...
class VLLMTeacher(nn.Module):
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                raise ImportError(
                    "httpx is required for VLLMTeacher. Install with: pip install httpx"
                )
        return self._client
    ...
    async def _get_logits_async(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        tasks = [get_single_logits(input_ids[i].tolist()) for i in range(batch_size)]
...
class VLLMTeacherWithPrefetch(VLLMTeacher):
    ...
    def prefetch(self, input_ids: torch.Tensor) -> None:
        ...
        self._prefetch_future = self._executor.submit(self._get_logits_sync, input_ids)
