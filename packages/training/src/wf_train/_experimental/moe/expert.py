...
from wf_train._experimental.moe.router import MoERouter, TopKRouter, IdentityRouter
...
class BitNetExpertFFN(nn.Module):
    """
    Single BitNet expert FFN (SwiGLU-style with ReLU squared).

    Architecture: FFN(x) = down(up(x) * relu^2(gate(x)))
