...
class WrinkleFreeLightningModule(pl.LightningModule):
    ...
    def forward(self, **batch) -> dict[str, Any]:
        ...
        # Validate input_ids range on first forward pass
        if not hasattr(self, "_input_validated"):
            input_ids = batch["input_ids"]
            vocab_size = getattr(self.model.config, "vocab_size", None)
            if vocab_size is not None and input_ids.max().item() >= vocab_size:
                logger.error(
                    f"input_ids max ({input_ids.max().item()}) >= vocab_size ({vocab_size})! "
                    f"Check mask_token_id or tokenizer compatibility."
                )
    ...
    def training_step(self, batch: dict[str, Any], batch_idx: int) -> torch.Tensor:
        ...
        if current_global_step > self._last_curriculum_step:
            ...
            # Get current phase name (needed for logging)
            ...
            if new_data_config and new_data_config != old_data_config:
                ...
                logger.info(f"[CURRICULUM] Switching to phase '{new_phase}' with data_config: {old_data_config} -> {new_data_config}")
    ...
    def configure_optimizers(self):
        ...
        if optimizer_type.lower() == "muon":
            ...
            logger.info(f"Muon optimizer: lr_muon={lr_muon}, lr_adam={lr_adam}")
