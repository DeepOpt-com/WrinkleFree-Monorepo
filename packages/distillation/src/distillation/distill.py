...
def is_hf_model_id(path_str: str) -> bool:
    """Check if the string is a HuggingFace model ID (e.g., 'microsoft/bitnet-b1.58-2B-4T-bf16')."""
    # HuggingFace model IDs are in format: org/model-name or just model-name
    # They don't start with gs://, /, or .
    if path_str.startswith("gs://"):
        return False
    if path_str.startswith("/") or path_str.startswith("."):
        return False
    # Check for org/model format or just model name without path separators
    if "/" in path_str and not path_str.count("/") > 1:
        # org/model format (single slash)
        return True
    # Could be a model ID without org prefix
    return False
...
def load_dlm_student(
    ...
    Args:
        checkpoint_path: Path to DLM checkpoint directory (local, gs://, or HuggingFace model ID)
    ...
    is_hf_hub = is_hf_model_id(checkpoint_str)

    # Handle GCS paths
    if checkpoint_str.startswith("gs://"):
        checkpoint_path = download_directory_from_gcs(checkpoint_str)
        path = Path(checkpoint_path)
    elif is_hf_hub:
        # HuggingFace Hub model ID - load directly
        path = None  # Signal that we'll load from hub
        logger.info(f"Loading DLM student model from HuggingFace Hub: {checkpoint_str}")
    else:
        path = Path(checkpoint_path)
    ...
    # Load DLM config if available (only for local/GCS paths)
    dlm_config = {}
    teacher_name = None
    model_config = {}
    ...
    # Determine model path/ID for loading
    model_path_or_id = str(path) if path is not None else checkpoint_str
    ...
    try:
        ...
        config = BitNetConfig.from_pretrained(model_path_or_id)
...
def run_distillation(
    ...
    distill_config = DistillationConfig(
        ...
        lr_muon=cfg.optimizer.get("lr_muon", 1e-3),  # MuonClip: Muon LR for main weights
        lr_adam=cfg.optimizer.get("lr_adam", 5e-5),  # MuonClip: Adam LR for bias/norm
        scheduler_type=cfg.scheduler.type,
        warmup_steps=cfg.scheduler.get("warmup_steps", 0),
        warmup_ratio=cfg.scheduler.get("warmup_ratio", 0.03),
    ...
    # Get mask_token_id for DLM training
    mask_token_id = None
    if student_type == "dlm":
        # Try to get from dlm_config first
        if "mask_token" in dlm_config:
            mask_token_id = dlm_config["mask_token"]
            logger.info(f"Using mask_token_id from DLM config: {mask_token_id}")
        # Fall back to tokenizer if available
        elif tokenizer is not None and hasattr(tokenizer, "mask_token_id") and tokenizer.mask_token_id is not None:
            mask_token_id = tokenizer.mask_token_id
            logger.info(f"Using mask_token_id from tokenizer: {mask_token_id}")
        else:
            # Use a sensible default for BitNet models (token ID 128002 is typical for mask)
            # This is a fallback - ideally should be specified in config
            mask_token_id = 128002
            logger.warning(f"No mask_token found in config or tokenizer, using default: {mask_token_id}")
...
def main(cfg: DictConfig) -> None:
    ...
    # Handle GCS paths (gs://) and HuggingFace model IDs - don't validate locally
    if checkpoint_path_str.startswith("gs://") or is_hf_model_id(checkpoint_path_str):
        checkpoint_path = checkpoint_path_str  # Keep as string for GCS/HuggingFace
    else:
